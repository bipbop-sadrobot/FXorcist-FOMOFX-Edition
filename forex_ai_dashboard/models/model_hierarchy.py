...
# Try to import lightgbm, fallback to sklearn if not available
try:
...
class ExecutorModel(BaseModel):
    """Low-level execution decision making using LightGBM or fallback."""
    ...
    async def train(self, X: pd.DataFrame, y: pd.Series, batch_size: int = 1000):
        ...
        try:
            if LIGHTGBM_AVAILABLE:
                ...
                # Sklearn GradientBoostingRegressor - train on full dataset
                self.model.fit(X, y)
                await self.log_event('batch_complete', {
                    'batch': 0,
                    'samples': len(X)
...
class ModelHierarchy:
    ...
    async def initialize_models(
        ...
        executor_metadata = ModelMetadata(
            name="executor_v1",
            version="1.0.0",
            type="lightgbm" if LIGHTGBM_AVAILABLE else "gradient_boosting",
            layer="executor",
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            # Get importance from strategist model (CatBoost)
            if hasattr(self.strategist.model, 'feature_importances_'):
                strategist_features = self.strategist.metadata.features
                strategist_importance = self.strategist.model.feature_importances_
                for feature, importance in zip(strategist_features, strategist_importance):
                    importance_scores[f"strategist_{feature}"] = float(importance)

            # Get importance from executor model (LightGBM or GradientBoosting)
            if hasattr(self.executor.model, 'feature_importances_'):
                executor_features = self.executor.metadata.features
                executor_importance = self.executor.model.feature_importances_
                for feature, importance in zip(executor_features, executor_importance):
                    importance_scores[f"executor_{feature}"] = float(importance)
