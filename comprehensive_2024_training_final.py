#!/usr/bin/env python3
"""
Comprehensive 2024 Forex Training Pipeline - Final Version
Processes all available 2024 data from multiple currency pairs for extensive training
Handles both CSV and semicolon-separated formats with robust NaN handling
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/comprehensive_2024_training_final.log"),
...
def create_features(df):
    """Create technical indicators and features for training with robust NaN handling"""
    df = df.copy()
    ...
    # Volume features
    df['volume_sma_5'] = df['volume'].rolling(window=5, min_periods=1).mean().fillna(df['volume'].mean())
    df['volume_ratio'] = (df['volume'] / df['volume_sma_5']).fillna(1)

    # Price momentum
    df['momentum_3'] = (df['close'] / df['close'].shift(3) - 1).fillna(0)
    df['momentum_5'] = (df['close'] / df['close'].shift(5) - 1).fillna(0)

    # High-Low range
    df['range'] = ((df['high'] - df['low']) / df['close']).fillna(0)
    df['range_sma_5'] = df['range'].rolling(window=5, min_periods=1).mean().fillna(0)

    # Lag features
    df['close_lag_1'] = df['close'].shift(1).fillna(df['close'])
    df['close_lag_2'] = df['close'].shift(2).fillna(df['close'])
    df['returns_lag_1'] = df['returns'].shift(1).fillna(0)

    # Target variable: next period return (don't shift yet)
...
def main():
    ...
    logger.info("🚀 Starting Comprehensive 2024 Forex Training - Final Version")
